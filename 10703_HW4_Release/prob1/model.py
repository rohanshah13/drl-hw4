...
class PENN(nn.Module):
    ...
    def get_loss(self, targ, mean, logvar):
        ...
        def loss_fn(mean, targ, logvar):
            eps = 1e-6
            loss = 0.5 * (logvar + (targ - mean) ** 2 / (torch.exp(logvar) + eps)).sum(dim=1).mean()
            return loss
        loss = loss_fn(mean, targ, logvar)
        return loss
    ...
    def train_model(self, inputs, targets, batch_size=128, num_train_itrs=5):
        ...
        # Train
        losses = []
        network_losses = [0 for _ in range(self.num_nets)]
        for i in trange(num_train_itrs):
            for batch_idx, (inputs, targets) in enumerate(loader):
                ...
                for i, output in enumerate(outputs):
                    mean, logvar = output
                    loss += self.get_loss(targets, mean, logvar)
                    network_losses[i] += loss.item()
                loss.backward()
                self.opt.step()
            # Get average loss for each network
            network_losses = [net_loss / len(dataset) for net_loss in network_losses]
            losses.append(network_losses)
